<script lang="ts">
  import { playerState, next, previous, playFromQueue, updateProgress } from '$stores/player';
  ...
  $: if (!hasAttemptedAutoResume && $playerState.queue.length > 0 && $playerState.current_index >= 0 && currentTrack) {
    console.log('[PlayerBar] Queue loaded, auto-resuming track:', currentTrack.title);
    hasAttemptedAutoResume = true;
    // Trigger the play-local event to resume
    window.dispatchEvent(new CustomEvent('jamarr:play-local', { detail: currentTrack }));
  }
  ...
  onMount(() => {
     ...
     window.addEventListener('jamarr:play-local', (e: CustomEvent) => {
         ...
         if (audio) {
             console.log('[PlayerBar] Setting audio src to:', `/api/stream/${track.id}`);
             ...
             if ($playerState.is_playing) {
                 audio.play().then(() => {
                     ...
                     console.warn("[PlayerBar] Auto-play blocked by browser, user must click play:", e.message);
     ...
     if (audio) {
         console.log('[PlayerBar] Audio element found, adding timeupdate and ended listeners');
         ...
         audio.addEventListener('timeupdate', () => {
             ...
             if (Math.floor(audio.currentTime) % 5 === 0 && Math.floor(audio.currentTime) !== Math.floor(oldProgress)) {
                 console.log('[PlayerBar] timeupdate #' + timeupdateCount + ':', audio.currentTime.toFixed(2), 'currentTrack:', currentTrack?.title);
             }
             
             checkPlayThreshold(); // Check if we should log to history
             
             // Update server with progress every 5 seconds
             if (currentTrack && Math.floor(audio.currentTime) - lastUpdateTime >= 5) {
                 console.log('[PlayerBar] Calling updateProgress with:', audio.currentTime.toFixed(2), !audio.paused);
  ...
  function togglePlay() {
    console.log('[PlayerBar] togglePlay called, audio.src:', audio?.src, 'audio.paused:', audio?.paused);
    if (!audio) {
    ...
    if (!audio.src || audio.src === window.location.href) {
      console.log('[PlayerBar] togglePlay: no src set, dispatching play-local event');
      if (currentTrack) {
        window.dispatchEvent(new CustomEvent('jamarr:play-local', { detail: currentTrack }));
      } else {
...
<div class="fixed bottom-0 w-full bg-surface-900 border-t border-white/10 p-4 text-white z-50">
  <div class="flex items-center justify-between max-w-[1700px] mx-auto">
    
    <!-- Track Info -->
    <div class="flex items-center gap-4 w-1/3">
      {#if currentTrack}
        <div class="relative h-14 w-14 flex-shrink-0 rounded bg-surface-800 overflow-hidden group">
            <img 
                src={currentTrack.art_id ? `/art/${currentTrack.art_id}` : '/assets/logo.png'} 
                alt="Art" 
                class="h-full w-full object-cover"
                on:error={(e) => { e.currentTarget.src = '/assets/logo.png'; }} 
            />
            <button class="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity" on:click={toggleQueue}>
                <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"></path></svg>
            </button>
        </div>
        <div class="min-w-0">
          <div class="font-medium truncate">{currentTrack.title}</div>
          <div class="text-sm text-white/60 truncate">{currentTrack.artist}</div>
          <div class="flex items-center gap-2 text-xs text-white/40 mt-0.5">
            ...
            {#if currentTrack.bit_depth && currentTrack.sample_rate_hz}
                <span>•</span>
                <span>{currentTrack.bit_depth}bit / {currentTrack.sample_rate_hz / 1000}kHz</span>
            {/if}
    ...
    <div class="flex flex-col items-center gap-2 w-1/3">
      ...
      <!-- Progress -->
      <div class="flex items-center gap-2 w-full max-w-md text-xs text-white/60">
        ...
        <input 
            ...
            on:input={(e) => { if(audio) audio.currentTime = parseFloat(e.currentTarget.value); }}
        />
    ...
    <div class="w-1/3 flex justify-end items-center gap-4">
        ...
        <button class="btn btn-ghost btn-sm btn-circle" title="Queue" on:click={toggleQueue}>
