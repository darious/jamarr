<script lang="ts">
  ...
  import { fetchTracks, refreshArtistMetadata, refreshArtistSingles } from '$lib/api';
  ...
  $: displayedSingles = (() => {
    ...
    const processed = singles.map(s => {
        ...
        let localAlbum = data.albums.find(a => a.album.toLowerCase() === s.title.toLowerCase());
        ...
        if (localAlbum) {
             ...
             const localTrack = tracks.find(t => t.title.toLowerCase() === s.title.toLowerCase());
             if (localTrack) {
                 // Construct a fake album object to allow navigation/artwork
                 localAlbum = {
                     album: localTrack.album,
                     artist_name: localTrack.artist,
                     art_id: data.albums.find(a => a.album === localTrack.album)?.art_id,
...
<section class="relative z-10 mx-auto flex w-full max-w-[1700px] flex-col gap-10 px-8 py-10">
  <div class="grid gap-8 md:grid-cols-[300px,1fr] items-start">
    <div class="relative aspect-square w-full max-w-[300px] rounded-2xl overflow-hidden shadow-2xl">
      <img
    ...
    <div class="space-y-6">
      <div class="space-y-2">
        <p class="pill w-max bg-white/10 text-white/70 backdrop-blur-md">Artist</p>
        <h1 class="text-4xl md:text-6xl font-bold tracking-tight">{artist?.name ?? data.name}</h1>
      </div>
      ...
      <div class="prose prose-invert max-w-none">
        <p class="whitespace-pre-line text-white/80 leading-relaxed line-clamp-[10] hover:line-clamp-none transition-all">
        ...
        {:else}
            {#each displayedTopTracks as track, idx}
              <div class="group flex items-center gap-4 px-4 py-3 hover:bg-white/5 transition-colors">
                ...
                <div class="h-12 w-12 flex-shrink-0 rounded bg-white/10 overflow-hidden relative">
                   {#if track.id && track.id > 0}
                     ...
                     <div class="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                        <button class="text-white hover:scale-110 transition-transform" on:click={() => playTrackById(track.id)}>
                ...
                <div class="flex-1 min-w-0">
                  <p class={`truncate text-sm font-semibold ${track.id > 0 ? 'text-white/90 group-hover:text-white' : 'text-white/30 line-through'}`}>{track.title}</p>
                  <div class={`flex items-center gap-2 text-xs mt-0.5 ${track.id > 0 ? 'text-white/50' : 'text-white/20 line-through'}`}>
    ...
    <div class="space-y-4 h-full flex flex-col">
      <div class="section-head">
        ...
        <div class="flex gap-2">
            <button class="btn btn-ghost btn-sm" on:click={playAllSingles} title="Play All Singles">
                ▶ Play All
            </button>
            <button class="btn btn-ghost btn-sm" on:click={refreshSingles} disabled={refreshingSingles}>
              ...
              <div class="group flex items-center gap-4 px-4 py-3 hover:bg-white/5 transition-colors">
                <div class="h-12 w-12 flex-shrink-0 rounded bg-white/10 overflow-hidden relative">
                   {#if single.art_id}
                     ...
                     {#if single.localId}
                        <div class="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                            <button class="text-white hover:scale-110 transition-transform" on:click={() => {
                                if (single.tracksToPlay && single.tracksToPlay.length > 0) {
                ...
                <div class="flex-1 min-w-0">
                  <button 
                    ...
                    on:click={() => single.localId && goto(`/album/${encodeURIComponent(data.name)}/${encodeURIComponent(single.title)}`)}
                  >
                    {single.title}
                  </button>
                  <div class={`flex items-center gap-2 text-xs mt-0.5 ${single.localId ? 'text-white/50' : 'text-white/20 line-through'}`}>
                      {#if single.localId}
                         {#if single.codec}
                         ...
                         {#if single.bit_depth && single.sample_rate_hz}
                             <span>{single.bit_depth}bit / {single.sample_rate_hz / 1000}kHz</span>
                             <span class="text-white/30">•</span>
                         {/if}
                      {/if}
                      <span>{single.date ? single.date.substring(0, 4) : 'Unknown'}</span>
                  </div>
                </div>
