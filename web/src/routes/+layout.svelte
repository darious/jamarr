<script lang="ts">
  ...
  async function scanLibrary() {
    ...
    try {
      ...
      isScanning = false;
      setTimeout(() => { scanMessage = ''; showSettings = false; }, 2000);
    }
  }
</script>
  ...
  <header class="sticky top-0 z-30 border-b border-white/5 bg-gradient-to-r from-black/70 via-surface-50/80 to-black/70 backdrop-blur-xl">
    <div class="mx-auto flex w-full max-w-[1700px] items-center justify-between px-6 py-3">
      <a href="/" class="flex items-center">
        <div class="flex h-12 w-12 items-center justify-center overflow-hidden rounded-xl border border-white/10 bg-white/5">
          <img src="/assets/logo.png" alt="Jamarr" class="h-full w-full object-contain p-2" />
        </div>
      </a>
      <div class="flex items-center gap-3">
        ...
        <div class="relative">
          ...
          {#if showSettings}
            <div class="absolute right-0 mt-2 w-56 rounded-lg border border-white/10 bg-surface-900 shadow-xl">
              <div class="p-2">
                <button
