...
<section class="relative z-10 mx-auto flex w-full max-w-[1500px] flex-col gap-8 px-8 py-10">
  <div class="grid gap-8 md:grid-cols-[300px,1fr] items-end">
    <div class="relative aspect-square w-full max-w-[300px] group rounded-2xl overflow-hidden shadow-2xl">
      <img
      ...
      <!-- Overlay Controls -->
      <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-4 backdrop-blur-sm">
          <button 
            class="btn btn-circle btn-primary btn-lg scale-90 hover:scale-100 transition-transform" 
            title="Play Album"
            on:click={playAll}
          >
          ...
          <button 
            class="btn btn-circle btn-ghost btn-lg bg-white/10 hover:bg-white/20 scale-90 hover:scale-100 transition-transform" 
            title="Add to Queue"
            on:click={addAllToQueue}
          >
            <svg class="h-8 w-8" fill="currentColor" viewBox="0 0 24 24"><path d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z"/></svg>
          </button>
      </div>

      {#if data.albumMeta?.is_hires}
        <img src="/assets/logo-hires.png" alt="Hi-res" class="absolute left-3 bottom-3 h-8 w-8 opacity-90" />
      {/if}
    </div>
    
    <div class="space-y-4 pb-2">
      <p class="pill w-max bg-white/10 text-white/70 backdrop-blur-md">Album</p>
      <h1 class="text-4xl md:text-6xl font-bold tracking-tight">{data.album}</h1>
      <div class="flex items-center gap-2 text-xl">
        <button class="font-medium hover:underline" on:click={() => goto(`/artist/${encodeURIComponent(data.artist)}`)}>
            {data.artist}
        </button>
        <span class="text-white/40">•</span>
        <span class="text-white/60">{data.albumMeta?.year ? data.albumMeta.year.substring(0, 4) : '—'}</span>
        <span class="text-white/40">•</span>
        <span class="text-white/60">{data.tracks.length} tracks, {totalDuration()} min</span>
      </div>
    </div>
  </div>
      ...
      {#each data.tracks as track, idx}
        <div class="group flex items-center gap-4 px-4 py-3 hover:bg-white/5 transition-colors">
          ...
          <div class="h-12 w-12 flex-shrink-0 rounded bg-white/10 overflow-hidden relative">
            ...
            />
             <div class="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                <button class="text-white hover:scale-110 transition-transform" on:click={() => setQueue([track], 0)}>
          ...
          <div class="flex-1 min-w-0">
            <p class="truncate text-sm font-semibold text-white/90 group-hover:text-white">{track.title}</p>
