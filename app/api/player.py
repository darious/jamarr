    ...
    if track_id and track_id > 0:
        try:
            ...
            async with db.execute(
                ...
                if existing:
                    existing_id, existing_ip = existing
                    # If existing log was from system (unlikely now that we disabled it) or needs update
                    # We might want to ensure client_id is captured if missing?
                    # The previous logic updated 127.0.0.1 IPs. We should keep that for older logs or if we re-enable auto-log.
                    # But crucially, we update client_id too.
...
class LogPlayRequest(BaseModel):
    track_id: int
    # client_id is passed via header, not body usually, but could be body. 
    # For now, we rely on header->dependency.
    # hostname field is deprecated/ignored.
