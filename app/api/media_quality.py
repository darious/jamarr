...
async def get_media_quality_items(
    ...
    if category == "primary":
        where_clauses.append(f"{tbl}.mbid IN (SELECT DISTINCT artist_mbid FROM artist_album WHERE type='primary')")
    ...
    if filter_type == "total":
        ...
        if filter_value == "None":
             ...
             where_clauses.append(f"{tbl}.artwork_id IN (SELECT id FROM artwork WHERE source ILIKE '%fanart%')")
        elif filter_value == "Spotify":
             where_clauses.append(f"{tbl}.artwork_id IN (SELECT id FROM artwork WHERE source ILIKE '%spotify%')")
        else:
             where_clauses.append(f"{tbl}.artwork_id IS NOT NULL")
             where_clauses.append(f"{tbl}.artwork_id NOT IN (SELECT id FROM artwork WHERE source ILIKE '%fanart%' OR source ILIKE '%spotify%')")
         ...
         if filter_value:
             ...
             idx = len(params) + 1
             where_clauses.append(f"{tbl}.mbid IN (SELECT entity_id FROM external_link WHERE type=${idx} AND entity_type=CASE WHEN '{category}'='album' THEN 'album' ELSE 'artist' END)")
             params.append(filter_value)

    elif filter_type == "missing_link_type":
         if filter_value:
             idx = len(params) + 1
             where_clauses.append(f"{tbl}.mbid NOT IN (SELECT entity_id FROM external_link WHERE type=${idx} AND entity_type=CASE WHEN '{category}'='album' THEN 'album' ELSE 'artist' END)")
             params.append(filter_value)
...
async def media_quality_summary(db: asyncpg.Connection = Depends(get_db)):
    ...
    stats["primary"]["total"] = await db.fetchval(f"SELECT COUNT(*) FROM artist ar WHERE {primary_filter_sql}")

    # For backgrounds, we need to join or filter
    stats["primary"]["with_background"] = await db.fetchval(f"""
    ...
    stats["album_stats"]["with_artwork"] = await db.fetchval("SELECT COUNT(*) FROM album WHERE artwork_id IS NOT NULL")
    ...
    return {"artist_stats": {"all": stats["all"], "primary": stats["primary"]}, "album_stats": stats["album_stats"]}
