...
class Scanner:
    ...
    async def scan_filesystem(self, root_path: str = None, force_rescan: bool = False):
            ...
            try:
                ...
                async for db in get_db():
                    ...
                    t_rows = await db.fetch(
                        f"SELECT id FROM track WHERE {limit_clause}", *params
                    )
                    track_ids = [r[0] for r in t_rows]

                    a_rows = await db.fetch(
                        f"SELECT DISTINCT release_group_mbid FROM track WHERE {limit_clause}",
                        *params,
                    )
                    ...
                    if not track_ids:
                        logger.info(
                            "Force Rescan Cleanup: No existing tracks found in this path."
                        )
                        continue
                    ...
                    res = await db.execute(
                        "DELETE FROM track_artist WHERE track_id = ANY($1::bigint[])",
                        track_ids,
                    )
                    ...
                    res = await db.execute(
                        "DELETE FROM image_map WHERE entity_type='track' AND entity_id = ANY($1::text[])",
                    ...
                    await db.execute(
                        "DELETE FROM track WHERE id = ANY($1::bigint[])", track_ids
                    )
                    ...
                    if album_mbids:
                        ...
                        res = await db.execute(
                            "DELETE FROM artist_album WHERE album_mbid = ANY($1::text[])",
                            album_mbids,
                        )
                        ...
                        res = await db.execute(
                            "DELETE FROM external_link WHERE entity_type='album' AND entity_id = ANY($1::text[])",
                            album_mbids,
                        )
                        ...
                        res = await db.execute(
                            "DELETE FROM image_map WHERE entity_type='album' AND entity_id = ANY($1::text[])",
                            album_mbids,
                        )
                        ...
                        # Delete albums
                        res = await db.execute(
                            "DELETE FROM album WHERE mbid = ANY($1::text[])",
                            album_mbids,
                        )
                        ...
                        res = await db.execute(
                            "DELETE FROM missing_album WHERE release_group_mbid = ANY($1::text[])",
                            album_mbids,
                        )
                    ...
                    if artist_mbids_to_clean:
                        ...
                        res = await db.execute(
                            "DELETE FROM external_link WHERE entity_type='artist' AND entity_id = ANY($1::text[])",
                            artist_list,
                        )
                        ...
                        res = await db.execute(
                            "DELETE FROM image_map WHERE entity_type='artist' AND entity_id = ANY($1::text[])",
                            artist_list,
                        )
                        ...
                        res = await db.execute(
                            "DELETE FROM artist_genre WHERE artist_mbid = ANY($1::text[])",
                            artist_list,
                        )
                        ...
                        res = await db.execute(
                            "DELETE FROM top_track WHERE artist_mbid = ANY($1::text[])",
                            artist_list,
                        )
                        ...
                        res = await db.execute(
                            "DELETE FROM similar_artist WHERE artist_mbid = ANY($1::text[])",
                            artist_list,
                        )
                    ...
                    # Build summary log
                    summary_parts = []
                    if counts.get("tracks"):
                        summary_parts.append(f"{counts['tracks']} tracks")
                    if counts.get("albums"):
                        summary_parts.append(f"{counts['albums']} albums")
                    if counts.get("artist_albums"):
                        summary_parts.append(
                            f"{counts['artist_albums']} artist-album links"
                        )
                    if counts.get("artist_links"):
                        summary_parts.append(
                            f"{counts['artist_links']} artist external links"
                        )
                    if counts.get("album_links"):
                        summary_parts.append(
                            f"{counts['album_links']} album external links"
                        )
                    if counts.get("tracks_top"):
                        summary_parts.append(
                            f"{counts['tracks_top']} top tracks/singles"
                        )
                    if counts.get("artist_genres"):
                        summary_parts.append(f"{counts['artist_genres']} genre tags")

                    summary = ", ".join(summary_parts) if summary_parts else "no data"
                    logger.info(
                        f"Force Rescan Cleanup: Purged {summary} for '{rel_root}' ({time.time() - start_t:.2f}s)"
                    )
        ...
        async for db in get_db():
            ...
            self.stats["total_estimate"] = (
                actual_count if actual_count > 0 else (db_count or 1000)
            )

            logger.info(
                f"Estimated file count: {self.stats['total_estimate']} (find said {actual_count}, db has {db_count})"
            )
            self.stats["current_status"] = "Scanning Files"

            await self._scan_recursive(
                root_path, artist_mbids, seen_paths, force_rescan
            )
            ...
            for mbid in all_mbids:
                await db.execute(
                    "INSERT INTO artist (mbid) VALUES ($1) ON CONFLICT (mbid) DO NOTHING",
                    mbid,
                )
                count += 1
                # if count % 100 == 0: await db.commit()

            # Run Consensus Logic to pick the best name from tags
            if all_mbids:
                await self._apply_artist_name_consensus(db, all_mbids)

            # Fallback: Update remaining null names using scan results (if consensus missed them for some reason)
            count = 0
            for mbid, name in artist_mbids:
                if mbid and name:
                    await db.execute(
                        "UPDATE artist SET name = $1 WHERE mbid = $2 AND (name IS NULL OR name = '')",
                        name,
                        mbid,
                    )
                    count += 1
            ...
            logger.info(
                f"Scanner finished: {self.stats['scanned']} scanned, {self.stats['added']} added, {self.stats['updated']} updated, {self.stats.get('deleted_relations', 0)} relations cleared."
            )
    ...
    async def _apply_artist_name_consensus(self, db, mbids):
        ...
        try:
            ...
            if int(updated_count) > 0:
                logger.info(
                    f"Consensus: Updated {updated_count} artist names based on tag popularity."
                )
        except Exception as e:
                            ...
                            for entry in entries:
                                if entry.is_dir():
                                    ...
                                    if ext in SUPPORTED_EXTENSIONS:
                                        ...
                                        if (
                                            self.scan_logger
                                            and self.stats["scanned"] % 10 == 0
                                        ):
                                            self.scan_logger.emit_progress(
                                                self.stats["scanned"],
                                                self.stats["total_estimate"],
                                                f"Scanning {entry.name}",
                                            )
    ...
    async def _process_file(self, path, db, artist_mbids, force_rescan, entry=None):
        try:
            ...
            if (
                ...
                and abs(cached_mtime.timestamp() - mtime) < 0.001
            ):
            ...
            values = [
                ...
                tags.get("codec"),
                tags.get("sample_rate_hz"),
                tags.get("bit_depth"),
                tags.get("bitrate"),
                tags.get("channels"),
                tags.get("label"),
                tags.get("artist_mbid"),
                tags.get("album_artist_mbid"),
                tags.get("track_mbid"),
                tags.get("release_track_mbid"),
                tags.get("release_mbid"),
                mb_rg_id,
                artwork_id,
            ]
            ...
            if artwork_id:
                if mb_rg_id:
                    await upsert_image_mapping(
                        db, artwork_id, "album", mb_rg_id, "album"
                    )
                else:
                    await upsert_image_mapping(
                        db, artwork_id, "track", track_id, "album"
                    )
            ...
            if mb_rg_id and album_title:
                ...
                try:
                    ...
                    await db.execute(
                        "DELETE FROM missing_album WHERE release_group_mbid = $1",
                        mb_rg_id,
                    )
                    get_api_tracker().track_processed("albums", mb_rg_id)
                except Exception as e:
                    logger.warning(
                        f"Error upserting album {album_title} ({mb_rg_id}): {e}"
                    )

            # 2. Handle Artists & Junctions
            if track_id:
                await self._process_track_artists(
                    db, track_id, tags, artist_mbids, mb_rg_id
                )
    ...
    async def _process_track_artists(
        self, db, track_id, tags, artist_mbids, mb_rg_id=None
    ):
        # Clear existing
        ...
        for mbid in ids:
            ...
            await db.execute(
                "INSERT INTO track_artist (track_id, artist_mbid) VALUES ($1, $2) ON CONFLICT (track_id, artist_mbid) DO NOTHING",
                track_id,
                mbid,
            )
            ...
            if tags.get("title"):
                ...
                for mbid in aa_ids:
                    ...
                    def normalize(s):
                        ...
                        s = s.lower().strip()
                        s = re.sub(
                            r"[\(\[][^\)\]]*(feat|with|remast|deluxe|edit|mix)[^\)\]]*[\)\]]",
                            "",
                            s,
                        )
                        s = re.sub(r"[^\w\s]", "", s)
                    ...
                    def fuzzy_score(s1, s2):
                        return difflib.SequenceMatcher(
                            None, normalize(s1), normalize(s2)
                        ).ratio()
    ...
    async def update_metadata(
        ...
        async for db in get_db():
            ...
            def has_selected_gaps(row, fetch_top, fetch_singles):
                ...
                if fetch_metadata and (
                    not name
                    or not str(name).strip()
                    or not sort_name
                    or not str(sort_name).strip()
                ):
                    return True
                if fetch_bio and (not bio or not str(bio).strip()):
                    return True

                if fetch_artwork:
                    if not art_id_existing or not image_url:
                        return True
                    elif art_source_existing == "spotify":
                        return True  # Always replace spotify (low quality usually)

                if fetch_links and (link_count or 0) == 0:
                    return True

                if fetch_top and (top_track_count or 0) == 0:
                    return True
                if fetch_singles and (single_count or 0) == 0:
                    return True

                if fetch_similar_artists and (similar_count or 0) == 0:
                    return True
            ...
            for row in rows:
                ...
                # Logic Check
                if missing_only and not has_selected_gaps(
                    row, refresh_top_tracks, refresh_singles
                ):
                    continue
                ...
                if missing_only:
                    if top_track_count > 0:
                        eff_refresh_top_tracks = False
                    if single_count > 0:
                        eff_refresh_singles = False
                ...
                has_meta_core = bool(
                    name and str(name).strip() and sort_name and str(sort_name).strip()
                )
                has_links = (link_count or 0) > 0

                if missing_only:
                    if has_meta_core:
                        eff_fetch_metadata = False
                    if has_bio:
                        eff_fetch_bio = False
                    if has_artwork:
                ...
                while True:
                    ...
                    while len(pending_tasks) < MAX_PENDING:
                        try:
                            ...
                            eff_refresh_top_tracks, eff_refresh_singles = (
                                flags[4],
                                flags[5],
                            )
                            eff_fetch_metadata, eff_fetch_bio, eff_fetch_links = (
                                flags[0],
                                flags[1],
                                flags[3],
                            )
                            ...
                            if missing_only and (
                                eff_refresh_top_tracks or eff_refresh_singles
                            ):
                                rows = await db.fetch(
                                    "SELECT type, count(*) FROM top_track WHERE artist_mbid=$1 GROUP BY type",
                                    mbid,
                                )
                                rows_top = rows
                                ...
                                for r_type, r_count in rows_top:
                                    if r_type == "top" and r_count > 0:
                                        has_top_tracks = True
                                    if r_type == "single" and r_count > 0:
                                        has_singles = True
                            ...
                            is_bio_only = eff_fetch_bio and not any(
                                [
                                    eff_fetch_metadata,
                                    eff_fetch_links,
                                    eff_fetch_artwork,
                            ...
                            if is_bio_only:
                                row = await db.fetchrow(
                                    "SELECT url FROM external_link WHERE entity_type='artist' AND entity_id=$1 AND type='wikipedia'",
                                    mbid,
                                )
                                row_wiki = row
                                if row_wiki:
                            ...
                            task = asyncio.create_task(
                                self._produce_metadata_update(
                                    semaphore,
                                    row,
                                    flags,
                                    local_release_group_ids,
                                    known_wikipedia_url,
                                    client,
                                )
                            )
                        ...
                        try:
                            ...
                            if res:
                                ...
                                if res["row"] and res["row"][0]:
                                    get_api_tracker().track_processed(
                                        "metadata_artists", res["row"][0]
                                    )
                                ...
                                if self.scan_logger:
                                    ...
                                    self.scan_logger.emit_progress(
                                        processed_count, total, f"Metadata: {last_name}"
                                    )
            ...
            if results_buffer:
                ...
                if self.scan_logger:
                    self.scan_logger.emit_progress(
                        processed_count, total, "Metadata: Complete"
                    )

            logger.info(
                f"Metadata update complete. Processed {processed_count} artists."
            )

    async def _produce_metadata_update(
        self,
        semaphore,
        row,
        flags,
        ...
        async with semaphore:
            ...
            if not self._stop_event.is_set():
                if eff_fetch_artwork and meta.get("image_url"):
                    art_download = await download_and_save_artwork(
                        meta["image_url"], art_type="artistthumb"
                    )
                if eff_fetch_artwork and meta.get("background_url"):
                    bg_download = await download_and_save_artwork(
                        meta["background_url"], art_type="artistbackground"
                    )
            ...
            try:
                ...
                if art_download:
                    artwork_id = await upsert_artwork_record(
                        ...
                        source_url=art_download.get("source_url")
                ...
                if bg_download:
                    bg_art_id = await upsert_artwork_record(
                        ...
                        source_url=bg_download.get("source_url")
                        or meta.get("background_url"),
                    )
                    if bg_art_id:
                        await upsert_image_mapping(
                            db,
                ...
                """,
                    ...
                    artwork_id,
                    datetime.fromtimestamp(meta.get("updated_at"), timezone.utc)
                    if isinstance(meta.get("updated_at"), (int, float))
                    else (meta.get("updated_at") or datetime.now(timezone.utc)),
                    mbid,
                )
                ...
                if eff_fetch_links:
                    await db.execute(
                        "DELETE FROM external_link WHERE entity_type='artist' AND entity_id=$1",
                        mbid,
                    )
                    artist_links = []
                    try:
                    ...
                    if meta.get("spotify_url"):
                        artist_links.append(("spotify", meta["spotify_url"]))
                    if meta.get("tidal_url"):
                        artist_links.append(("tidal", meta["tidal_url"]))
                    if meta.get("qobuz_url"):
                        artist_links.append(("qobuz", meta["qobuz_url"]))
                    if meta.get("wikipedia_url"):
                        artist_links.append(("wikipedia", meta["wikipedia_url"]))
                    if meta.get("homepage"):
                        artist_links.append(("homepage", meta["homepage"]))
                    if meta.get("lastfm_url"):
                        artist_links.append(("lastfm", meta["lastfm_url"]))
                    if meta.get("discogs_url"):
                        artist_links.append(("discogs", meta["discogs_url"]))
                ...
                # 4. Top Tracks
                should_refresh = (
                    eff_refresh_top_tracks and not SPOTIFY_SCANNING_DISABLED
                )
                if should_refresh:
                    await db.execute(
                        "DELETE FROM top_track WHERE artist_mbid=$1 AND type='top'",
                        mbid,
                    )
                ...
                if eff_refresh_singles:
                    await db.execute(
                        "DELETE FROM top_track WHERE artist_mbid=$1 AND type='single'",
                        mbid,
                    )
                ...
                if eff_fetch_similar_artists:
                    await db.execute(
                        "DELETE FROM similar_artist WHERE artist_mbid=$1",
                        mbid,
                    )
                ...
                if meta.get("genres"):
                    await db.execute(
                        "DELETE FROM artist_genre WHERE artist_mbid=$1",
                        mbid,
                    )
                    for g in meta["genres"]:
                        await db.execute(
                            "INSERT INTO artist_genre (artist_mbid, genre, count, updated_at) VALUES ($1, $2, $3, NOW()) ON CONFLICT (artist_mbid, genre) DO NOTHING",
                            mbid,
                            g["name"],
                            g["count"],
                        )
                ...
                if eff_fetch_metadata:
                    ...
                    for release in all_releases:
                        ...
                        tagged_release_ids = []
                        rows = await db.fetch(
                            "SELECT DISTINCT release_mbid FROM track WHERE release_group_mbid = $1 AND release_mbid IS NOT NULL",
                            r_mbid,
                        )
                        rows_tagged = rows
                        ...
                        if r_release_ids:
                            await db.execute(
                                "UPDATE track SET release_group_mbid=$1 WHERE release_mbid = ANY($2::text[])",
                                r_mbid,
                                r_release_ids,
                            )
                        ...
                        if r_primary_release:
                            ...
                            mb_release_link = (
                                f"{get_musicbrainz_root_url()}/release-group/{r_mbid}"
                            )

                        link_payloads = []
                        if mb_release_link:
                            link_payloads.append(
                                {"type": "musicbrainz", "url": mb_release_link}
                            )
                        link_payloads.extend(r_links)

                        await db.execute(
                            "DELETE FROM external_link WHERE entity_type='album' AND entity_id=$1",
                            r_mbid,
                        )
        ...
        async for db in get_db():
            ...
            async with httpx.AsyncClient(
                ...
                for row in artists:
                    ...
                    if self.scan_logger:
                        self.scan_logger.emit_progress(
                            processed, total, f"Checking: {current_name}"
                        )
                    ...
                    await db.execute(
                        "DELETE FROM missing_album WHERE artist_mbid = $1",
                        mbid,
                    )
                    ...
                    try:
                        # Fetch Albums
                        mb_albums = await fetch_artist_release_groups(
                            mbid, "album", client
                        )
                        ...
                        for album in mb_albums:
                            ...
                            if not tidal_url:
                                try:
                                    ...
                                    found_url = tidal_client.find_album_match(
                                        current_name, album["title"], want_year
                                    )
                                    if found_url:
                                        tidal_url = found_url
                        ...
                        logger.error(
                            f"Error checking missing albums for {current_name}: {e}"
                        )
