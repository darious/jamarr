...
@pytest.mark.asyncio
async def test_playlist_lifecycle(client, db, auth_token):
    """
    ...
    headers = {"Cookie": f"jamarr_session={auth_token}"}

    # 1. Create Playlist
    resp = await client.post("/api/playlists", json={"name": "My Test Playlist", "description": "A description"}, headers=headers)
    ...
    # 2. Add Tracks
    # Add 9001, 9002
    resp = await client.post(f"/api/playlists/{playlist_id}/tracks", json={"track_ids": [9001, 9002]}, headers=headers)
    assert resp.status_code == 200
    
    # Add 9001 again (duplicate)
    resp = await client.post(f"/api/playlists/{playlist_id}/tracks", json={"track_ids": [9001]}, headers=headers)
    assert resp.status_code == 200

    # 3. Get Playlist Detail
    resp = await client.get(f"/api/playlists/{playlist_id}", headers=headers)
    assert resp.status_code == 200
    detail = resp.json()
    ...
    resp = await client.post(f"/api/playlists/{playlist_id}/reorder", json={"allowed_playlist_track_ids": new_order}, headers=headers)
    assert resp.status_code == 200
    ...
    resp = await client.delete(f"/api/playlists/{playlist_id}/tracks/{pt_ids[0]}", headers=headers)
    assert resp.status_code == 200
    ...
    # 6. Delete Playlist
    resp = await client.delete(f"/api/playlists/{playlist_id}", headers=headers)
    assert resp.status_code == 200
    ...
    await client.post("/api/playlists", json={"name": "Listable Playlist"}, headers=headers)
    ...
    new_pid = (await client.get("/api/playlists", headers=headers)).json()[0]["id"] # Get most recent
    await client.post(f"/api/playlists/{new_pid}/tracks", json={"track_ids": [tid_art]}, headers=headers)

    resp = await client.get("/api/playlists", headers=headers)
    assert resp.status_code == 200
    listed = resp.json()
...
@pytest.mark.asyncio
async def test_public_private_playlist(client, db, auth_token):
    headers = {"Cookie": f"jamarr_session={auth_token}"}
    
    # Create private (default)
    resp = await client.post("/api/playlists", json={"name": "Private Playlist"}, headers=headers)
    priv_id = resp.json()["id"]
    
    # Create public
    resp = await client.post("/api/playlists", json={"name": "Public Playlist", "is_public": True}, headers=headers)
    pub_id = resp.json()["id"]
    ...
    resp = await client.get(f"/api/playlists/{priv_id}", headers=headers)
